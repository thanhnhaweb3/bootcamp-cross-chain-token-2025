...
import { MYTOKEN_ABI, BASE_SEPOLIA_TOKEN_ADDRESS, BASE_SEPOLIA_ROUTER_ADDRESS, CHAIN_SELECTORS, ROUTER_ABI } from '@/constants/contract'

export default function CCTBaseSepoliatoSaigonTestnet() {
  // Cố định số lượng token để chuyển
  const fixedAmount = 66;
  ...
  const { data: allowance } = useReadContract({
    ...
    args: [address || '0x0000000000000000000000000000000000000000', BASE_SEPOLIA_ROUTER_ADDRESS],
  ...
  const createCCIPMessage = useCallback((amountBigInt: bigint) => {
    ...
    const extraArgs = concat([
      ...
      encodeAbiParameters([{ type: 'uint256' }], [BigInt(0)]) as `0x${string}`
    ]);
    ...
    const message = {
      ...
      extraArgs: extraArgs as `0x${string}`
    };
  ...
  const calculateFee = useCallback(async () => {
    if (!address || chainId !== 84532) {
      setError(chainId !== 84532 ? 'Please switch to Base Sepolia network' : 'Please connect your wallet');
      return;
    }
    ...
    try {
      ...
      const data = await estimateFee({
        address: BASE_SEPOLIA_ROUTER_ADDRESS as `0x${string}`,
        abi: ROUTER_ABI,
        functionName: 'getFee',
        args: [
          CHAIN_SELECTORS.SAIGON,
          message
        ],
      });
  ...
  const handleApprove = useCallback(async () => {
    ...
    if (chainId !== 84532) { // Base Sepolia chain ID
      setError('Please switch to Base Sepolia network')
      return
    }

    setError('')
    const amountBigInt = BigInt(fixedAmount * 10 ** 18) // 66 tokens
    ...
    try {
      approve({
        ...
        args: [BASE_SEPOLIA_ROUTER_ADDRESS as `0x${string}`, amountBigInt],
      })
    } catch (err: unknown) {
  ...
  const handleTransfer = useCallback(async () => {
    ...
    try {
      ...
      console.log("CCIP Send Parameters:", {
        ...
        destinationChainSelector: CHAIN_SELECTORS.SAIGON.toString(),
        message: {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-6">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Base Sepolia ➡️ Saigon Transfer</h2>
      <div className="bg-blue-50 p-4 rounded-lg mb-4 text-blue-800">
        <p className="font-medium">Quick transfer of {fixedAmount} {tokenSymbol} from Base Sepolia to Saigon</p>
      ...
      <div className="mt-4 bg-gray-50 p-4 rounded-lg">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center">
            <div className={`mr-2 w-3 h-3 rounded-full ${chainId === 84532 ? 'bg-green-500' : 'bg-red-500'}`}></div>
            <span>{chainId === 84532 
              ? `Connected to Base Sepolia network` 
              : `Not connected to Base Sepolia network`}</span>
          </div>
        </div>
      </div>
        ...
        <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-600 mt-2">
              The tokens should arrive in your wallet on Saigon in a few minutes. Check your balance on the Saigon network.
            </p>
          </div>
        </div>
      )}
